...
(function () {
  ...
  /* Namen und Sprueche stammen aus rangobjekt.js (STUFEN) — dort
     stehen sie neben der Geometrie, die sie beschreiben. Hier nur
     eine Kopie mit ASCII-Sicherung, falls das Modul einmal fehlt:
     dann bleibt die Galerie trotzdem lesbar statt leer.            */
  var NAMEN = ['staub', 'schwarm', 'bindung', 'zwilling',
               'schale', 'sphaere', 'klarheit', 'stille'];
  var TITEL = (window.RANGOBJEKT && window.RANGOBJEKT.STUFEN)
    ? window.RANGOBJEKT.STUFEN.map(function (s) { return s.name; })
    : ['STAUB', 'SCHWARM', 'BINDUNG', 'ZWILLING', 'SCHALE', 'SPHÄRE', 'KLARHEIT', 'STILLE'];
  var SPRUCH = (window.RANGOBJEKT && window.RANGOBJEKT.STUFEN)
    ? window.RANGOBJEKT.STUFEN.map(function (s) { return s.satz; })
    : ['Allein und unfertig.', 'Masse ohne Ordnung.', 'Die erste Symmetrie.',
       'Entworfen statt zufällig.', 'Das erste Leuchten.', 'Höhepunkt der Komplexität.',
       'Nichts mehr verborgen.', 'Beinahe Stillstand.'];
  var ROEMISCH = ['I', 'II', 'III', 'IV'];
  /* Fallback-Farbe je Stufe, falls rangobjekt.js fehlt — sonst
     kommt die echte Palette aus RANGOBJEKT.stufenFarbe().          */
  var FARBFALL = ['#6f6660', '#8a6b57', '#c85a35', '#e8492b',
                  '#f0673a', '#f79150', '#f9bb74', '#ffe3b4'];
  function stufenfarbe(i) {
    return (window.RANGOBJEKT && window.RANGOBJEKT.stufenFarbe)
      ? window.RANGOBJEKT.stufenFarbe(i + 1) : FARBFALL[i];
  }
  ...
  function bandBauen() {
    ...
    var karten = NAMEN.map(function (n, i) {
      return '<article class="rg-karte" data-i="' + i + '">' +
               ...
               '<div class="rg-bild" style="background:radial-gradient(circle at 38% 34%,' +
               'rgba(255,255,255,.9),' + stufenfarbe(i) + ' 55%,rgba(0,0,0,.85) 100%)"></div>' +
    ...
    lauf.addEventListener('click', function (e) {
      ...
      var i = +k.dataset.i;
      if (window.__rang3d && window.__rang3d.morphZu) window.__rang3d.morphZu(i + 1);
    });
  ...
  Kammer.prototype.stueckeStellen = function () {
    ...
    return Promise.all(NAMEN.map(function (n, i) {
      return window.__rang3d.modellHolen(i + 1).then(function (knoten) {
        if (!knoten) return;
