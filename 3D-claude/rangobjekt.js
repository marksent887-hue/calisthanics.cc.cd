/* rangobjekt.js — Die acht Rang-Objekte von FLOW, prozedural.
 *
 * Alles hier ist Mathematik: Kugeln, Kegel, Torusbahnen, ein Diamantgitter.
 * Kein Byte Base64-Geometrie. Der Grund ist nicht Sparsamkeit, sondern
 * Ehrlichkeit — das Objekt IST eine Formel, es soll auch eine bleiben.
 *
...
export const FELDNAMEN = ['Zug', 'Druck', 'Rumpf', 'Beine', 'Stütz', 'Balance'];
...
export function erzeugeRangObjekt(einstellung = {}) {
...
export function beleuchteSzene(szene, stufe) {
  const w = stufenWerte(stufe);
  const t = w.t;
...
export function erzeugeUmgebung(renderer) {
  const c = document.createElement('canvas');
...
export function stufenFarbe(stufe) { return PALETTE[klemme(stufe, 1, 8) - 1]; }
export function stufenGrenzen(stufe) { return GRENZEN[klemme(stufe, 1, 8) - 1]; }
